...
And when ~POSTID~ was created these rules were written:
...
- only users that have bid on an auction, and the auction creator,
...
We get a table with the total bids that references an auction and a bidder. So we just set the privacy rules as:
